-- ZOQO Alpha, phase 0: schema only. Scheduling (cron.schedule calls) is
-- deliberately NOT in this file — see supabase/alpha-cron-apply.sql.example
-- for why: those calls embed the CRON_SECRET in plain SQL, and a migration
-- is checked into git. Extensions are safe to enable here since they carry
-- no secret.
...
-- price_history: retention raised to 30 days (code change, not DDL — see
-- src/app/api/cron/evaluate-triggers/route.ts). Downsampling for rows older
-- than 48h needs a single bounded SQL statement (not a JS loop pulling rows
-- client-side), so it's a DB function the route calls via .rpc(): for each
-- asset and each 5-minute bucket, keep only the earliest row and delete the
-- rest.
create or replace function downsample_price_history() returns void
language sql
as $$
  ...
  using (
    select ctid,
           row_number() over (
             partition by asset_id, date_trunc('hour', ts) + floor(extract(minute from ts) / 5) * interval '5 minutes'
             order by ts asc
           ) as rn
    from price_history
    where ts < now() - interval '48 hours'
  ) bucketed
  where t.ctid = bucketed.ctid and bucketed.rn > 1;
$$;
