...
/**
 * A light top nav for /referrals — shares its logo/nav/deposit/stats/bell/
 * auth chrome with TopNav via @/components/trade/HeaderChrome, but doesn't
 * carry the BTC/duration selector that page is tightly coupled to.
 */
export function ReferralsTopNav() {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-[60px] items-center gap-3 border-b bg-surface/90 px-3 backdrop-blur-md sm:px-4">
      ...
      <HeaderNav activeAutomations={activeAutomations} alphaUnread={alphaUnread} className="mx-auto" />
