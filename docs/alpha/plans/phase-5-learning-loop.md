...
## Scope in
- `src/lib/alpha/metrics.ts` — pure math, Vitest-first: bootstrap 95% ROI CI, hit rate, Brier, RPS, CLV (`taken_implied/closing_implied - 1`), max drawdown, Sharpe-like ratio on daily P&L. Every function takes plain arrays of settled-order records, no I/O — same "pure math module" discipline as `kelly.ts`/`risk.ts`.
